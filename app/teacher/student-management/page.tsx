...
export default function StudentManagement() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch("/api/teacher/student-management")
        if (res.ok) {
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold text-lg overflow-hidden">
                    {student.imageUrl ? (
              ...
              <div className="grid md:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-foreground mt-2">
                    {student.avgGrade >= 90 ? "Excellent" : student.avgGrade >= 80 ? "Good" : student.avgGrade >= 60 ? "Average" : "Needs Improvement"}
                  </p>
                </div>
              </div>

              <div className="flex gap-3">
                <Link href={`/teacher/communication?studentId=${student.id}`} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 font-medium transition-colors">
                  Send Message
                </Link>
                {/* 
