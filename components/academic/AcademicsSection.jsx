"use client";

import Image from "next/image";

export default function AcademicSection() {
  return (
    <section id="career" className="max-w-6xl mx-auto bg-[#F7F6F3] px-4 sm:px-5 md:px-6 py-10 sm:py-14 md:py-20 lg:py-24 font-['Montserrat']">
      <div className="flex flex-col lg:flex-row items-center gap-8 sm:gap-10 md:gap-12 lg:gap-16 xl:gap-20">
        
        {/* Image Composition Container */}
        <div className="relative w-full max-w-full sm:max-w-100 md:max-w-110 lg:max-w-125 aspect-square group">
          
          {/* 1. Background Image (Navy Overlay - #1F2A44) */}
          <div className="absolute top-0 left-0 w-[80%] h-[75%] overflow-hidden rounded-none border border-[#D4A437]/10">
            <div className="absolute inset-0 bg-[#1F2A44]/80 mix-blend-multiply z-10" />
            <Image
              src="/new/image28.jpeg"
              alt="Student in classroom"
              fill
              className="object-cover grayscale transition-transform duration-1000 group-hover:scale-105"
              sizes="(max-width: 640px) 80vw, (max-width: 768px) 70vw, (max-width: 1024px) 50vw, 40vw"
            />
          </div>

          {/* 2. Foreground Image (Natural Colors with Gold Accent) */}
          <div className="absolute bottom-0 right-0 w-[75%] h-[70%] z-20 shadow-2xl overflow-hidden rounded-none border-8 border-white">
            <Image
              src="/new/image38.jpeg"
              alt="Students learning outdoors"
              fill
              className="object-cover transition-transform duration-1000 group-hover:scale-110"
              sizes="(max-width: 640px) 75vw, (max-width: 768px) 65vw, (max-width: 1024px) 45vw, 35vw"
            />
            {/* Subtle overlay to match editorial style */}
            <div className="absolute inset-0 bg-[#1F2A44]/5 group-hover:bg-transparent transition-colors duration-500" />
          </div>

          {/* Decorative Gold Detail */}
          <div className="absolute -bottom-4 -right-4 w-24 h-24 border-r-2 border-b-2 border-[#D4A437]/30 z-0 hidden md:block" />
        </div>

        {/* Text Content */}
        <div className="w-full lg:w-1/2 space-y-6 text-[#222222]">
          <div className="w-12 h-1 bg-[#D4A437] mb-6" />
          <p className="text-base sm:text-lg md:text-xl leading-relaxed font-light opacity-90 italic">
            Because students experience all aspects of life within the
            community, they come to understand academics as a natural and
            necessary part of life. Just as they need nourishment and social
            interaction, they also need academic enrichment in order to grow.
          </p>
          <p className="text-base sm:text-lg md:text-xl leading-relaxed font-semibold text-[#1F2A44]">
            This disposition motivates students to fully engage so they can not
            only gain the knowledge they need to progress in their chosen field,
            but the learning strategies they need to be fully ready for life.
          </p>
        </div>
      </div>
    </section>
  );
}